...
for FotosTomografos in os.listdir(PastaFotasOriginais):
    ...
    ImagemGirada = ImagemRotacionar.rotate(-89.9)                                     # Girar a imagem em 90 graus no sentido horário
...
for arquivo in lista_arquivos:
    ...
    reconstructed_image = 60 - reconstructed_image.astype(np.uint8)
...
lista_arquivos = os.listdir(PastaPlanosAxiais)

fig = plt.figure(figsize=(8, 8))
ax = fig.add_subplot(111, projection='3d')

for i, arquivo in enumerate(lista_arquivos):
    caminho_imagem = os.path.join(PastaPlanosAxiais, arquivo)
    plano_axial = io.imread(caminho_imagem, as_gray=True)
    plano_axial = resize(plano_axial, (image_size_3d, image_size_3d), anti_aliasing=True)

    x, y = np.meshgrid(np.arange(image_size_3d), np.arange(image_size_3d))
    z = np.full_like(x, i)

    ax.plot_surface(x, y, z, facecolors=plt.cm.gray(plano_axial), shade=False)

ax.set_xlim([0, image_size_3d])
ax.set_ylim([0, image_size_3d])
ax.set_zlim([0, len(lista_arquivos)])
...
plt.savefig('Imagem3D.png', bbox_inches = 'tight', pad_inches = 0.1, dpi = 1000)
plt.show()
